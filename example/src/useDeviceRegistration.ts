...
/**
 * Registers this device's push token with the support server.
 *
 ...
 * exactly that key.
 *
 * Registration is re-run whenever the platform reissues the token. A stale
 * token fails silently — at APNs, and at FCM — so the push simply never
 * arrives and nothing points at the cause.
 */
export function useDeviceRegistration(externalUserId: string): void {
  useEffect(() => {
    ...
    return watchPushToken(({ platform, token }) => {
      void (async () => {
        try {
          const response = await fetch(url, {
            ...
            body: JSON.stringify({
              ...
              platform,
              token,
              // Only meaningful for APNs: a development build is signed for
              // the sandbox, and sending a sandbox token to production (or the
              // reverse) fails at Apple with a device-token mismatch, which is
              // easy to misread as a bad token. FCM has no such split.
